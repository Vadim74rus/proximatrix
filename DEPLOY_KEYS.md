...
1. На сервере 77.221.156.12 открыт порт **27017** (UFW и при необходимости панель хостинга).
2. В docker-compose для MongoDB можно оставить только внутреннюю сеть; если нужен доступ снаружи — пробросить порт, например:
...
3. С другого сервера использовать URI:

   ```
   mongodb://proximatrix:ВАШ_ПАРОЛЬ@77.221.156.12:27017/proximatrix?authSource=admin
   ```

4. API по адресу `http://77.221.156.12:9090` уже доступен с любого сервера (при открытом 9090). Все ключи — **X-API-Key** и при необходимости логин/пароль MongoDB — храните в безопасности и не коммитьте в репозиторий.
