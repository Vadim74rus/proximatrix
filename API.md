...
Статусы: `connected`, `disconnected`, `blocked`.
...
Система автоматически отслеживает IP-адреса подключений для каждого секрета:

- **Первое подключение**: IP автоматически добавляется в список разрешенных
- **Одновременные подключения с разных IP**: Если один секрет используется одновременно с разных IP-адресов, пользователь автоматически отключается (`enabled: false`)
- **Уведомления**: При обнаружении подозрительной активности в логах выводится подробная информация
- **Восстановление**: После проверки администратор может включить пользователя обратно через `POST /api/users/{id}/enable`
- **Сброс IP**: После включения можно сбросить список разрешенных IP через `POST /api/users/{id}/reset-ips`

Все подключения логируются в `connectionHistory` пользователя.
