...
Если видите ошибку **"❌ Не подключено: fetch failed"** или **"ECONNREFUSED"**, выполните на сервере Proximatrix (77.221.156.12):
...
- Проверит и запустит контейнер, если он не запущен
- Откроет порт 9090 в UFW, если он закрыт
- Проверит доступность API
- Выведет информацию для подключения
